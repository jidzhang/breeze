...
1.03 罗光瑜 afterShow方法，是默认的去调用decorate的函数实现的
1.04 罗光瑜 queryData方法，在查询父亲的时候，把排序字段去掉
1.05 罗光瑜 metadata中的type的处理，修正
* @description  CMS控制器基本接口 注意:所有control都需要自行引用自己所需要的资源文件。
*公有方法是能力，私有方法是辅助显示功能的      
*/
define(function(require, exports, module) {
    var FW = require("breeze/framework/js/BreezeFW");
    require("./CMSMgrSelector");
    FW.register({
        "name": "CMSMgrControl",
        /**
        *@function
        ...
        */
        ...
        "public": {
                    ...
                    for (var i = 0; i < decorates.length; i++) {
                        ...
                        var resource = {
                            param: {
                                ...
                                binddomid: this.param.binddomid
                            },
            ...
            *@description 处理和转换查询出来的元数据格式,原来的数据是datadesc中的一个json字符串，现在要解开这个字符串
            *@param __data 待转换数据
            *@return json
            */
            "handleMetaDataBefore": function(__data) {
                ...
                //初始化排序数组
                //--元数据后面要按照指定顺序排序，这个决定了后面对象在页面上的显示顺序
                var sortArr = [];
                ...
                for (var mName in metadata) {
                    ...
                    //type的处理
                    //--这个思考很久了，改版方式还是统一在control实施修改，变成单值对象，算法是原来是单值就单值，原来不是单值就
                    //--根据type处理，最后用defaule，如果还是没有，那就空着
                    if (/\{/.test(oneMData.type)) {
                        var urlType = modType = FW.use().getParameter("type");
                        var cid = FW.use().getParameter("cid");

                        if (cid) {
                            modType = urlType + "_mod";
                        } else {
                            modType = urlType + "_add";
                        }

                        var typeObj = eval("(" + oneMData.type + ")");
                        oneMData.type = (typeObj[modType] || typeObj[urlType] || typeObj["default"] || "Hidden");
                    }
                    //if(是select情况){处理下拉选项
        ...
        "private": {
            ...
            *@description 显示组件，这个是在页面上被调用的函数
            *@param name 组件名称
            *@return 组件的字符串
            */
            ...
            *该类将自动去调用所有decorate的aftershow的方法，如果子类要重写，那么一定要调用supper的该方法
            *@param data 要处理的实际数据
            */
            "afterShow": function(data) {
                ...
                if (decorates) {
                    ...
                    for (var i = 0; i < decorates.length; i++) {
                        //获取app
                        var app = FW.getApp(decorates[i].instance);
                        //调用app的aftershow
                        app.initAfterShow(data);
                    }
                    //}
                }
                //}
