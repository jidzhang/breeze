...
* @version 0.52 2015-11-05 罗光瑜修改，把doServer首次访问改成异步形式
* @version 0.53 2015-12-28 罗光瑜修改，doServer支持ajax的跨域请求
* @version 0.54 2015-12-31罗光瑜修改，调试情况下也要支持跨域url
* @version 0.55 2016-02-27罗光瑜修改，page级别的gadget创建实例后，手动调用了onCreate，其实这个是不用手动调用的,这个需要增加createType在app中，以区分是手工添加还是程序创建
* @version 0.56 2016-04-09罗光瑜修改，动态创建app后，MY变量不能共享，否则内部封装的值会被改版，破坏封装性
...
//					}
	...
	function _createFWTools(__app){
		...
		*/
		var realShow = function(__viewId,__data,__target,method){
			...
			if (tmpDom == null || tmpDom.length == 0){
				tmpDom = $("#"+_$app.id);
				if (tmpDom.length == 0 && /object/i.test(typeof(_$app.dom))){
					tmpDom = _$app.dom;
				}
			}
			var target= tmpDom;
			...
			//2016-05-03罗光瑜修改对app没视图情况的判断
			var src = _$app.view && _$app.view[__viewId] || __viewId;
			if (src == null){
				target.html("");
				return;
			}
		...
		*});
		*/
		result.doServer = _APPAPI.doServer || function(__serverName,__package,__data,__callback,__paramUrl){
			//2013-12-23罗光瑜修改，如果传入的__callback是null则是同步请求
			return _obj.doServer(__serverName,__package,__data,__callback,_$app,__paramUrl);
		};
	...
	_obj.go = function(__url,pageGadget,$callBack){
		...
		$(
			function(){
				...
				for (var name in _app){
					//2016-02-27罗光瑜修改，page级别的gadget创建实例后，手动调用了onCreate，其实这个是不用手动调用的
					if (_app[name].createType == "appCreate"){
						continue;
					}
					_app[name].onCreate();
				};
	...
	*});
	*/
	_obj.doServer=function(__serverName,__package,__data,__callback,__callbackThisObj,__paramUrl){
		...
		if (false){
			...
			var temp = _createPost(__paramUrl);
	...
	*/
	_obj.createApp = function(__appName,__gadgetName,__resourceAPP,__useAppName){
		...
		//2016-04-09罗光瑜修改，动态创建app后，MY变量不能共享，否则内部封装的值会被改版，破坏封装性
		...
		//2016-02-27罗光瑜修改，page级别的gadget创建实例后，手动调用了onCreate，其实这个是不用手动调用的,这个需要增加createType在app中，以区分是手工添加还是程序创建
		app.createType = "appCreate";
