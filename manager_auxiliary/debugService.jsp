...
<html lang="en">
<head>
	<link rel="shortcut icon" href="./img/icon/debugservice.jpg" >
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<!--强制使用浏览器模式-->
<meta http-equiv="X-UA-Compatible" content="IE=edge" />
<!--强制使用文本模式-->
<title>service调试</title>
...
<script>
	...
	seajs.use(['${B}manager_auxiliary/service/test' ],
					function(a) {
						a.go("${B}<%=target%>");
						window.FW = a;
						...
						function linkage(objdata,selectDom){  //联动函数，change时候给下级赋值
							...
							if(sn == curTr.find("select").length-1){
								...
								for (var i = 0;i<nameArr.length;i++){
									curTr.find("select").eq(sn+1).append("<option value='"+nameArr[i]+"'>"+nameArr[i]+"</option>");
								}
						...
						for(var prop in testData){
							packageArr.push(prop);
							
						}
						packageArr.sort(function(a,b){
								return a < b ?-1:1;
						});
						for (var i = 0;i<packageArr.length;i++){
							_selectDom.append("<option value='"+packageArr[i]+"'>"+packageArr[i]+"</option>");
						}
						_selectDom.change();
