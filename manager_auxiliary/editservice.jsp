...
<script>
	...
	//2016-08-21 罗光瑜 名称也要排序
	var tmp = {};
	pkgSortArr.sort(function(a,b){
    		return a < b ?-1:1;
    });
	for (var i = 0 ;i<pkgSortArr.length;i++){
		tmp[pkgSortArr[i]] = serviceNames[pkgSortArr[i]];
	}
	serviceNames = tmp ;
	...
	<div class="page-content clearfix">
		...
		-->
			<div id="view_selectFlow">
				...
				<br><br>
				<select id="v_flows" class="chosen-select">
			...
			<div id="view_selectPackage">
				...
				<br><br>
				<select id="v_package" class="chosen-select" onchange="FireEvent.changePackage()">
					<!--$for(var name in data.sall){-->
						<option value="${_}{name}">${_}{name}</option>
					<!--$}-->
				</select>
				<select id="v_name" class="chose-select">
