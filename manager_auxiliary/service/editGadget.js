	...
	FW.register({
			...
			initNew: function(txt) {
				...
				if (/object/.test(typeof(txt))) {
					if (txt){						
						...
						classMod.setMod(txt);
						this.MY.textMod.setClassMod(classMod);
					}					
				}
		...
		private:{
			...
			*/
			changeTips:function(word,isObj){
				...
				showTmp += "内部: ${data.m.replace(/,/g,'，')}<br/>\n";
		...
		FireEvent: {
			...
			changeView:function(text){
				var className = this.MY.classMod.getMod().name;
				$("#viewdir").val("./resource/"+className+"/"+text+".tpl");
			}
		},
		TrigerEvent: {
