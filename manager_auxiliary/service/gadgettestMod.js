define(function(require, exports, module) {
	...
	FW.register({
		...
		public: {
			 ...
			 * @param requirePath
			 */
			setRequirePath:function(requirePath){
				this.MY.requirePath = requirePath;
			},
			/**
			 * 设置测试名称
			 * @param testName
...
console.log("193:null-readStr:"+readStr+allStr.charAt(pIdx+1));
					...
					while (/[ ]|[\s]|[\n]|[\r]/i.test(readStr)) {
						//罗光瑜2016-08-31修改原来的pIdx++导致连续加了，因为一开始的时候就已经加1了
						readStr = allStr.charAt(pIdx++); 
					}
					//回调后会重新加，所以读到后索引要退一个，变成当前的，不能是下一个
					result = this.API.private("pBaseName", allStr, --pIdx , result, isArray);
					//}else if (pidx读入的是:){
				} else if (readStr == ":") {
