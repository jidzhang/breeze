        ...
        "private": {
            ...
            */
            "parserJavaClassFunction": function(lv1Array, idx) {
                ...
                var processCVar = function(objArr, idx) {
                    var result = {};
                    var one = objArr[idx];
                    if (one.type != "l") {
                        return null;
                    ...
                    if (one.type == "b" && one.content == "<") {
                        ...
                        while (true) {
                            ...
                            one = objArr[idx];
                            if (one.type == 'l') {
                                var subResult = processCVar(objArr, idx);
                ...
                while (true) {
                    ...
                    //else if(return状态处理){处理return状态
                        ...
                        if (one.type == "l") {
                            var returnTypeObj = processCVar(declearArray, declearIdx);
                    ...
                    //else if(处理name状态){处理name状态
                        ...
                        if (one.type == "l") {
                            ...
                            if (declearArray[declearIdx].type != 'b' || declearArray[declearIdx].content != '(') {
                                console.log("解析函数体的name后面出错了，碰到应该是(但没找到");
                                return null;
                            }
                            declearStatus = "var";
                        } else {
                            console.log("解析函数体的name出错了，碰到非字符");
                            return null;
                        }
                    }
                    //}
                    //else if(var){处理beginvar
                        ...
                        if (one.type == "b" && one.content == "(") {
                            ...
                            var tResult = processCVar(declearArray, declearIdx);
                            if (tResult == null) {
                                console.log("解析函数参数中的成员变量出错");
                                return null;
                            }
                            ...
                            if (one.type != 'l') {
                                console.log("解析出错，参数类型定义后面应该是变量名");
                                return null;
                            }
            ...
            */
            "parserJavaClassFunctionBodyNormal": function(lv1Struct, idx, destArray) {
                ...
                else if (lv1Struct[idx].type == "comments") {
                    //写注释写代码
                    result.command = lv1Struct[idx].content[0] && lv1Struct[idx].content[0].content || "";
            ...
            */
            "parserJavaClassFunctionBodyIf": function(lv1Struct, idx, destArray, isElse) {
                ...
                //}
                //else if(不是else情况){创建新的并将加入到destArray中
                if (!isElse || destArray.length == 0) {
                    //创建新的并将加入到destArray中
                ...
                nextIdx = resultIdx + 1;
                if (resultIdx < lv1Struct.length - 1 && lv1Struct[resultIdx + 1].type == "code") {
                    var codeStr = this.API.private('parserJavaClassFunctionBodyCode', lv1Struct[resultIdx + 1].content);
                    if (/^[\s\r\n\t]*\}[\r\s\n\t]*$/.test(codeStr) || /^[\s\t\r\n]*$/.test(codeStr)) {
                        nextIdx++;
                    }
                }
            ...
            */
            "parserJavaClassFunctionBody": function(lv1Struct, idx, destArray, parent) {
                ...
                //else{进一步进行注释分支判断
                    ...
                    //if(是if分支){处理if分支
                    if (/^\s*(if|如果).+/.test(content)) {
                        //进行if分子处理
                        this.API.private('parserJavaClassFunctionBodyIf', lv1Struct, idx, destArray);
                    }
                    //}
                    ...
                    //else if(如果是else if的情况){处理else if
                        ...
                        //处理else if的情况
                        this.API.private('parserJavaClassFunctionBodyIf', lv1Struct, idx, destArray, true);
                    }
                    //}
                    //else if(循环情况){处理循环
            ...
            */
            "parserJavaClassFunctionBodyCycle": function(lv1Struct, idx, destArray) {
                ...
                if (nextIdx < lv1Struct.length && lv1Struct[nextIdx].type == "code" && lv1Struct[nextIdx].content) {
                    var lastcode = this.API.private('parserJavaClassFunctionBodyCode', lv1Struct[nextIdx].content);
                    if (/^[\s\r\n]*\}/.test(lastcode) || /^[\s\t\r\n]*$/.test(lastcode)) {
                        nextIdx++;
                    }
                }
            ...
            *@param lv1Struct lv1Struct的结构体：
            ...
            注释内容，就是/**...的注释
            其中versiong的写法@version 0.01 罗光瑜 词法分析类
            描述部分的写法格式：@description  这是一个专用于词法分析的类，不做别的只做词法分析。注意可以换行吗，换行后头一个字符是*
            *@return {
            version:[{
            author: "lgy",
            description: "xxx",
            ...
            */
            "parserJavaClassNote": function(lv1Struct) {
                //toDo
            }
        }
    },
